# Splicer: How Wasm components enable truly pluggable middleware (and much more). #


| Architecture           | Binding Time                                | Observability Granularity                              | Failure Isolation                                                           | Language Portability               | Interface Specialization                              |
|------------------------|---------------------------------------------|--------------------------------------------------------|-----------------------------------------------------------------------------|------------------------------------|-------------------------------------------------------|
| Sidecar                | Deploy-time                                 | Types of protocol (L7), raw bytes (L4)                 | Separate process, workload survives                                         | All languages                      | 1 per protocol at type level; unbounded at byte level |
| eBPF                   | Runtime (dynamic attach)                    | Syscalls, kernel events, network packets               | Separate process, workload survives                                         | All languages                      | 0 typed; unbounded at raw / syscall only              |
| API Gateway            | Deploy-time                                 | Types of protocol                                      | Separate process, workload survives                                         | All languages                      | 1 per protocol at type level                          |
| In-language middleware | Compile-time                                | Types of protocol                                      | Same process, workload crashes                                              | 1 (tied to language and framework) | N typed within a protocol                             |
| Weaving / rewriting    | Build- (rewriting) or load-time (JVM Agent) | Types of protocol                                      | Rewriting: workload crashes, Agent: observable in workload, but not a crash | 1 (tied to language and framework) | N typed within a protocol                             |
| Splicer                | Build-time                                  | Typed WIT values at arbitrary WIT interface boundaries | Same wasm instance, failed request, but workload survives                   | All wasm languages                 | N typed WIT interfaces, polyglot                      |
...
In fact, there's an entire architecture that platform maintainers leverage to workaround this pervasive issue called [sidecars]. This architecture decouples the application from the platform, which allows the sidecar to be updated independent of the application (this even works for polyglot services). However, sidecars have been proven to bear a significant cost [[1], [2]].
...
Right now, the tool only supports adapting middleware components that don't require any reflection on the payload of the interface. Read-only reflection and read-write reflection are going to be tackled next!
...
1. implement a suite of middlewares that can be reused across heterogeneous interfaces
