...
/// Stage every tier-3/4 builtin crate (discovered by scanning
/// `builtins/*/manifest.toml` for `tier = 3` or `4`) under
/// `$OUT_DIR/embedded-strategies/<name>/`, and emit
/// `$OUT_DIR/embedded_strategies.rs` so `src/strategies.rs` can
/// `include!` the matching `include_dir!` statics + `EMBEDDED` slice.
/// Accepts either `Cargo.toml` or `Cargo.toml.embed` at the source
/// and rewrites the strategy's `splicer-tool-sdk` dep to drop the
...
/// Tiers whose builtins are source-distributed (staged into OUT_DIR
/// and compiled at splice time). Tiers not listed here ship as
/// pre-built wasm pulled from OCI. Extend when a new source-dist
/// tier comes online.
const STRATEGY_TIERS: &[i64] = &[3, 4];

/// Scan `builtins_root/*/manifest.toml` and return the names of
/// crates whose `[builtin].tier` is in [`STRATEGY_TIERS`]. Builtins
/// without a manifest (host-side helpers like `builtin-protocol`)
/// are skipped.
...
/// Emit a Rust source file that declares one `static <UPPER>: Dir<'_>`
/// per staged strategy plus the `EMBEDDED` slice tying names to
/// directories. `src/strategies.rs` `include!`s this so adding a new
/// tier-3/4 builtin is a pure source-tree edit -- the registry
/// updates automatically on next build.
