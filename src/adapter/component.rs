...
//!    that branches: it picks one of three "paths" depending on
//!    what context it has about the downstream interface:
//!    - **Path (S)** — `split_imports` was supplied, so copy the
//!      downstream split's raw type/import/alias bytes verbatim and
//!      either reuse the handler import (if the split imports it)
//!      or build a fresh handler instance type referencing the
//!      shared types from the raw sections.
//!    - **Path (A)** — the interface has resource types and named
//!      type exports; emit a separate types-instance import and an
//!      `alias outer` handler instance type that refers back to it.
//!    - **Path (B)** — no resources; emit one self-contained
...
//! 7.  **`canon lower`** — lower the hook funcs, the downstream funcs,
...
//! for now (`emit_hook_func_types`, `build_downstream_inst_type`,
