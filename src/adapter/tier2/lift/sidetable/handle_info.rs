...
/// Direct/RetptrPair handle results are Phase 3 — they'll add a
/// `per_result_single_fill` slot mirroring flags.
...
/// One `handle-info` entry per `Cell::Handle` (param plan or
/// compound result plan). Direct handle results land in phase 3.
...
impl<'a> HandleInfoBuilder<'a> {
    ...
    fn append_plan(&mut self, plan: &LiftPlan) -> (Option<SymRef>, Vec<Option<HandleRuntimeFill>>) {
        ...
        for (cell_pos, op) in plan.cells.iter().enumerate() {
            ...
            // `id` stays zero — the wrapper patches it per call.
